...
def send_subscription_reminders():
    ...
    qs = Subscription.objects.filter(is_active=True, expired_at__date=almost_expire.date())
    for sub in qs:
    ...
    expired = Subscription.objects.filter(is_active=True, expired_at__lt=now)
    for sub in expired:
        sub.is_active = False
        sub.save()
